...
プラグイン開発に役立つさまざまなツールが用意されています。それらの一部は開発環境 ([xdebug](http://xdebug.org/)、[PHPCS](https://github.com/WordPress/WordPress-Coding-Standards) など) で実行されますが、WordPress 内で直接実行され、開発や問題の診断に役立つ優れたツールもいくつかあります。本章ではブラウザ内ツールについて説明します。
